...
cdef extern:
    void optical_depth_blr_ring(double *tau_gg_blr, double *eps, double *z_jet,
                                double *redshift, double *L_Hbeta,
                                double *R_Hbeta)
...
def tau_blr(
    ...
    """
    Calculate the absorption optical depth on BLR spherical shell
...
def tau_blr_ring(
    ...
    """
    Calculate the absorption optical depth on BLR ring
    ...
    z_jet
        Distance from black hole to photon production zone in units of the
        radius of the Lyman α emitting BLR ring.
    redshift
        Redshift of emitting blazar. Set to zero, if you want the energies in
        the reference frame of the host galaxy.
    L_Hbeta
        Luminosity of the Hβ line in erg/sec.
    R_Hbeta
        Radius of the Hβ line emitting ring.
